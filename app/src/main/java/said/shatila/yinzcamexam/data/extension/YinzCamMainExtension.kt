    ...
    return sections.map { section ->
        GameSection(
            ...
            games = section.games?.map { dto ->
                val game = team?.let { dto.toGame(it, dto.type.equals(GameResult.FINAL.gameType)) }
...
fun GameDTO.toGame(homeTeam: TeamDTO, isFinal: Boolean): Game {
    return Game(
        ...
        gameStateDate = if (isFinal) gameState.orEmpty() else DateHelpers.formatDateTime(
            isoTimestamp = date?.timestamp.orEmpty(),
        ...
        dateText = DateHelpers.formatDateTime(
            isoTimestamp = date?.text.orEmpty(),
            outputPattern = "EEE, MMM dd"
        ),
